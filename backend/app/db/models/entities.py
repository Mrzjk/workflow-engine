...
class Agent(Timestamped):
    __tablename__="agents"; id: Mapped[str]=mapped_column(String(36),primary_key=True,default=uid); name: Mapped[str]=mapped_column(String(255)); description: Mapped[str|None]=mapped_column(Text); config: Mapped[dict]=mapped_column(JSON,default=dict)
...
class AgentRun(Base):
    __tablename__="agent_runs"; id: Mapped[str]=mapped_column(String(36),primary_key=True,default=uid); agent_id: Mapped[str]=mapped_column(String(36)); workflow_version_id: Mapped[str]=mapped_column(String(36)); input: Mapped[str]=mapped_column(Text); output: Mapped[dict|None]=mapped_column(JSON); status: Mapped[str]=mapped_column(String(32),default="queued"); started_at: Mapped[datetime|None]=mapped_column(DateTime); finished_at: Mapped[datetime|None]=mapped_column(DateTime)
class NodeRun(Base):
    __tablename__="node_runs"; id: Mapped[str]=mapped_column(String(36),primary_key=True,default=uid); run_id: Mapped[str]=mapped_column(ForeignKey("agent_runs.id")); node_id: Mapped[str]=mapped_column(String(128)); node_type: Mapped[str]=mapped_column(String(64)); input: Mapped[dict|None]=mapped_column(JSON); output: Mapped[dict|None]=mapped_column(JSON); status: Mapped[str]=mapped_column(String(32)); error: Mapped[str|None]=mapped_column(Text); duration: Mapped[float|None]=mapped_column(Float); started_at: Mapped[datetime|None]=mapped_column(DateTime); finished_at: Mapped[datetime|None]=mapped_column(DateTime)
